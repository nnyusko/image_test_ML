
import pandas as pd
import numpy as np
import lightgbm as lgb
import xgboost as xgb
import catboost as ctb
from sklearn.model_selection import StratifiedKFold
from sklearn.metrics import f1_score
from sklearn.preprocessing import LabelEncoder, StandardScaler
from sklearn.neighbors import KNeighborsClassifier
from sklearn.pipeline import Pipeline

# 1. 원본 데이터 불러오기
print("원본 데이터를 불러옵니다 (train.csv, test.csv)...")
train = pd.read_csv('assets/train.csv')
test = pd.read_csv('assets/test.csv')
submission = pd.read_csv('assets/sample_submission.csv')

# 레이블 인코딩
le = LabelEncoder()
train['target'] = le.fit_transform(train['target'])

# 데이터 준비
train_x = train.drop(columns=['ID', 'target'])
train_y = train['target']
test_x = test.drop(columns=['ID'])

# 2. 앙상블할 모델 정의 (KNN 추가 및 파이프라인 구성)
print("\n앙상블 모델을 정의합니다 (LGBM, XGB, CatBoost, KNN)...")

SEED = 42

# 각 모델을 스케일러와 파이프라인으로 묶어줍니다.
pipe_lgbm = Pipeline([('scaler', StandardScaler()), ('model', lgb.LGBMClassifier(random_state=SEED, reg_alpha=0.1, reg_lambda=0.1))])
pipe_lgbm_tuned = Pipeline([('scaler', StandardScaler()), ('model', lgb.LGBMClassifier(random_state=SEED, n_estimators=200, learning_rate=0.1, num_leaves=50, reg_alpha=0.1, reg_lambda=0.1))])
pipe_xgb = Pipeline([('scaler', StandardScaler()), ('model', xgb.XGBClassifier(random_state=SEED, use_label_encoder=False, eval_metric='mlogloss'))])
pipe_cat = Pipeline([('scaler', StandardScaler()), ('model', ctb.CatBoostClassifier(random_state=SEED, verbose=0))])
pipe_knn = Pipeline([('scaler', StandardScaler()), ('model', KNeighborsClassifier(n_neighbors=5))]) # n_neighbors는 대표적인 하이퍼파라미터

models = {
    'lgbm': pipe_lgbm,
    'lgbm_tuned': pipe_lgbm_tuned,
    'xgb': pipe_xgb,
    'cat': pipe_cat,
    'knn': pipe_knn
}

# 3. 교차 검증 기반 예측 확률 생성
N_SPLITS = 5
skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)

oof_preds = {}  # Out-of-Fold 예측 확률 저장
test_preds = {} # 테스트 데이터 예측 확률 저장

print(f"\n{N_SPLITS}-Fold 교차 검증을 시작합니다...")

for model_name, model in models.items():
    print(f"===== {model_name} 모델 학습 및 예측 중... =====")
    
    oof_model_preds = np.zeros((len(train), len(train_y.unique())))
    test_model_preds = np.zeros((len(test), len(train_y.unique())))

    for fold, (train_idx, val_idx) in enumerate(skf.split(train_x, train_y)):
        print(f"  Fold {fold+1}/{N_SPLITS}...")
        
        X_train, y_train = train_x.iloc[train_idx], train_y.iloc[train_idx]
        X_val, y_val = train_x.iloc[val_idx], train_y.iloc[val_idx]

        model.fit(X_train, y_train)

        oof_model_preds[val_idx] = model.predict_proba(X_val)
        test_model_preds += model.predict_proba(test_x) / N_SPLITS

    oof_preds[model_name] = oof_model_preds
    test_preds[model_name] = test_model_preds
    
    f1 = f1_score(train_y, np.argmax(oof_model_preds, axis=1), average='macro')
    print(f"  => {model_name} OOF F1 Score: {f1:.4f}\n")


# 4. 소프트 보팅 앙상블
print("소프트 보팅 앙상블을 수행합니다...")

oof_ensemble_preds = np.mean([preds for preds in oof_preds.values()], axis=0)
ensemble_f1 = f1_score(train_y, np.argmax(oof_ensemble_preds, axis=1), average='macro')
print(f"\n>> 최종 앙상블 (KNN 포함) OOF F1 Score: {ensemble_f1:.4f}")

# 5. 제출 파일 생성
print("제출 파일을 생성합니다...")
test_ensemble_preds = np.mean([preds for preds in test_preds.values()], axis=0)
final_preds = np.argmax(test_ensemble_preds, axis=1)

submission['target'] = le.inverse_transform(final_preds)
submission.to_csv('./soft_voting_knn_submit.csv', index=False, encoding='utf-8-sig')

print("\nsoft_voting_knn_submit.csv 파일 생성이 완료되었습니다.")
print(f"예상 F1 Score: {ensemble_f1:.4f}")
